...
const SetSchema = z.object({ class_id: z.string().uuid(), weekday_ids: z.array(z.string().uuid()) });
...
export async function PUT(req: Request, { params }: { params: Promise<{ schoolId: string }> }) {
  const { schoolId } = await params; try { if (!(await allowed(schoolId))) return NextResponse.json(error("Unauthorized"), { status: 401 }); const p = SetSchema.safeParse(await req.json()); if (!p.success) return NextResponse.json(error("Choose a class and weekdays"), { status: 400 }); return NextResponse.json(success(await setClassWeekdays(schoolId, p.data.class_id, p.data.weekday_ids))); } catch (e) { return failure(e); }
}
