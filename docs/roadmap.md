...
- ✅ Fee Defaulters — list students with unpaid/partial invoices; summary cards (total defaulters, outstanding amount); month filter (defaults to current month), class filter, debounced search (name, reg no, father CNIC, mobile, invoice no); paginated table with invoice/student/remaining details; print list button (opens print-optimized HTML in new tab); API at `GET /api/fees/[schoolId]/defaulters`
