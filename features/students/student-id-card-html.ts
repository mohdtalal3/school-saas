...
function cardHtml(student: StudentWithClass, school: School, theme: IdCardTheme): string {
  ...
  const photoInner = student.photo_url
  ...
  const rows: { label: string; value: string }[] = [
    { label: "Reg No", value: escapeHtml(student.registration_no) || "—" },
    { label: "Class", value: escapeHtml(student.class_name) || "—" },
    { label: "Father", value: escapeHtml(student.father_name) || "—" },
    { label: "Mobile", value: escapeHtml(student.mobile) || "—" },
    { label: "DOB", value: formatDate(student.date_of_birth) || "—" },
    { label: "Blood", value: escapeHtml(student.blood_group) || "—" },
  ];
  ...
  <div class="card" style="--accent:${accentColor};--accent-dark:${accentDark};--gold:${goldColor};--text:${textColor};--bg:${bgColor};">
    <div class="header">
      ...
      <div class="header-badge">STUDENT ID</div>
    </div>
    ${watermark}
    <div class="content">
...
export function buildStudentIdCardsHtml(
  ...
  const pagesHtml = pages
    .map(
      (pageStudents) =>
        `<div class="page">${pageStudents
          .map((s) => cardHtml(s, school, theme))
          .join("")}</div>`
    )
    .join("");
...
<head>
<meta charset="UTF-8">
<title>${escapeHtml(school.name)} — Student ID Cards</title>
